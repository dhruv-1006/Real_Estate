  ...
  return (
    <div className='absolute top-0 left-0 w-full z-10'>
        <div className='container mx-auto flex justify-between items-center py-4 px-6 md:px-20 lg:px-32 bg-transparent'>
            ...
            <button className='hidden md:block bg-white px-8 py-2 rounded-full text-black '>Sign up</button>
            <img onClick={()=> setShowmobileMenu(true)} src={assets.menu_icon} className='md:hidden w-7 cursor-pointer' alt="" />
        </div>

        {/* ------------ Mobile Menu ------------ */}

        <div className={`md:hidden ${showmobileMenu ? 'fixed w-50' : 'h-0 w-0' }  right-0 top-0 bottom-0 overflow-hidden bg-white transition-all`}>
